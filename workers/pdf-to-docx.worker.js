...
async function init() {
  ...
  // Load wheels sequentially to avoid concurrent WebAssembly compilation memory spikes
  self.postMessage({ type: 'status', message: 'Loading core parser (numpy)...' });
  await pyodide.loadPackage(basePath + 'numpy-2.2.5-cp313-cp313-pyodide_2025_0_wasm32.whl');

  self.postMessage({ type: 'status', message: 'Loading parser dependencies...' });
  await pyodide.loadPackage(basePath + 'typing_extensions-4.12.2-py3-none-any.whl');
  await pyodide.loadPackage(basePath + 'packaging-24.1-py3-none-any.whl');

  self.postMessage({ type: 'status', message: 'Loading structural compiler (lxml)...' });
  await pyodide.loadPackage(basePath + 'lxml-5.4.0-cp313-cp313-pyodide_2025_0_wasm32.whl');

  self.postMessage({ type: 'status', message: 'Loading vector extractor (PyMuPDF)...' });
  await pyodide.loadPackage(basePath + 'pymupdf-1.26.3-cp313-none-pyodide_2025_0_wasm32.whl');

  self.postMessage({ type: 'status', message: 'Loading font synthesizers...' });
  await pyodide.loadPackage(basePath + 'fonttools-4.56.0-py3-none-any.whl');

  self.postMessage({ type: 'status', message: 'Loading layout formats (docx)...' });
  await pyodide.loadPackage(basePath + 'python_docx-1.2.0-py3-none-any.whl');

  self.postMessage({ type: 'status', message: 'Loading image processor (opencv)...' });
  await pyodide.loadPackage(basePath + 'opencv_python-4.11.0.86-cp313-cp313-pyodide_2025_0_wasm32.whl');

  self.postMessage({ type: 'status', message: 'Installing converters (pdf2docx)...' });
  await pyodide.loadPackage(basePath + 'pdf2docx-0.5.8-py3-none-any.whl');

  self.postMessage({ type: 'status', message: 'Initializing offline engine...' });
...
def pdf_convert():
    """Convert PDF to DOCX with high-performance layout reconstruction."""
    cv = Converter("input.pdf")
    # Get page count from Converter's internal fitz doc (no extra open)
    page_count = len(cv.fitz_doc)
    
    # Disable heavy vector shape contour reconstruction (speeds up layout conversion by 10x!)
    # While fully maintaining text block alignments, columns, tables, and spacing!
